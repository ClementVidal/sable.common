...
class CCodePackage(object) :
    ...
    def GetBuildTargetList( self, buildConfig ) :
        ...
        if self.IsAgregated :  
            buildFilePath = self.GetWorkspace().GetBuildFileDir()
    ...
    def GetName( self ) :
        name = os.path.normpath( self.Path ).partition( "Source" )[2]
        if name == "" :
            LibLog.Error("Error: Invalid package path : "+self.Path )
            return ""
...
class CBuildTarget(object) :
    ...
    def GetCompiledObjectPath( self, extension ):
        ...
        targetname = os.path.splitext( os.path.basename( self.Path ) )[0]
        compiledTargetPath = self.GetBuildConfig().GetBuildDir()+"/"+targetname+extension

        return os.path.normpath( compiledTargetPath )
